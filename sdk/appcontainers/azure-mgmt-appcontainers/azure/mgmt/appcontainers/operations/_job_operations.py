...
def build_run_request(resource_group_name: str, job_name: str, subscription_id: str, **kwargs: Any) -> HttpRequest:
    _headers = case_insensitive_dict(kwargs.pop("headers", {}) or {})
    _params = case_insensitive_dict(kwargs.pop("params", {}) or {})
...
class JobOperations:
    ...
    def _executions_initial(
        ...
        request = build_executions_request(
            resource_group_name=resource_group_name,
            job_name=job_name,
            subscription_id=self._config.subscription_id,
            api_version=api_version,
            template_url=self._executions_initial.metadata["url"],
            headers=_headers,
            params=_params,
        )
        request = _convert_request(request)
    ...
    def begin_executions(
        ...
        :return: An instance of LROPoller that returns an iterator like instance of either
         ContainerAppJobExecutions or the result of cls(response)
        :rtype:
         ~azure.core.polling.LROPoller[~azure.core.paging.ItemPaged[~azure.mgmt.appcontainers.models.JobExecution]]
        :raises ~azure.core.exceptions.HttpResponseError:
        """
        ...
        def prepare_request(next_link=None):
            if not next_link:
                ...
                request = build_executions_request(
                    ...
                    subscription_id=self._config.subscription_id,
                    api_version=api_version,
                    template_url=self.begin_executions.metadata["url"],
                    headers=_headers,
                    params=_params,
                )
                request = _convert_request(request)
        ...
        cont_token: Optional[str] = kwargs.pop("continuation_token", None)
        if cont_token is None:
            raw_result = self._executions_initial(
                resource_group_name=resource_group_name,
                job_name=job_name,
                api_version=api_version,
                cls=lambda x, y, z: x,
